    ...
    public class RPGCharacterInputController : MonoBehaviour
    {
        ...
        private void Inputs()
        {
            try
            {
                ...
                lookInput = new Vector3(Input.mousePosition.x, 0f, Input.mousePosition.y);
            }
            catch (Exception)
            {
        ...
        private void Look()
        {
            if (lookInput.magnitude > 0.1f)
            {
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    Vector3 lookTarget = targetPosition - transform.position;
                    lookTarget.y = 0f;
                    rpgCharacterController.SetFaceInput(lookTarget.normalized);
                }
            }
            else
            {
                rpgCharacterController.SetFaceInput(Vector3.zero);
            }
        }
    }
}
	...
	public class RPGCharacterInputController : MonoBehaviour
    {
        ...
        private void Inputs()
        {
	        try {
		        ...
		        inputFace = Input.GetMouseButton(0);
        ...
        private void Facing()
        {
			...
			if (HasFacingInput()) {
                if (inputFace || Input.GetMouseButton(0))
                {
