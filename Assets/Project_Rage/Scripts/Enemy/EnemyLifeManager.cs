...
public class EnemyLifeManager : MonoBehaviour
{
    ...
    // Событие для оповещения об уничтожении врага
    public event System.Action<EnemyLifeManager> OnDeath;
    ...
    public void TakeDamage(float damage)
    {
        ...
        if (currentHealth <= 0f && !isDead)
        {
            ...
            if (enemyController3 != null)
                enemyController3.StopMovement();

            if (enemySwordAttack != null)
                enemySwordAttack.enabled = false;

            // Вызываем событие уничтожения врага
            OnDeath?.Invoke(this);
